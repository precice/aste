...
def plotErrorBest(dfo):
    select = [
        'nn', 'np', 'tps', 'compact-tps-c2-n3-separate',
        'compact-tps-c2-n20-separate'
    ]
    ...
    plt.grid()
    plt.savefig("turbine-small-error-best.pdf")
...
def plotErrorLRBF(dfo):
    ...
    plt.gca().invert_xaxis()
    plt.grid()
    plt.savefig("turbine-small-error-localrbf.pdf")


def plotErrorRBFPoly(dfo):
    select = [
        'gaussian-n3-on',
        'gaussian-n3-separate',
        'gaussian-n5-on',
        'gaussian-n5-separate',
        'compact-tps-c2-n5-on',
        'compact-tps-c2-n5-separate',
        'compact-tps-c2-n5-on',
        'compact-tps-c2-n5-separate',
    ]
    df = dfo[dfo['mapping'].isin(select)]
    ...
    plt.gca().invert_xaxis()
    plt.grid()
    plt.savefig("turbine-small-error-poly.pdf")


def plotMemory(dfo):
    select = [
        'nn', 'np', 'tps', 'compact-tps-c2-n20-separate',
        'compact-tps-c2-n10-separate', 'compact-tps-c2-n5-separate',
...
def plotComputeMappingTime(dfo):
    ...
    df = dfo[dfo["mapping"].isin([
        ...
        "compact-tps-c2-n3-separate",
        "compact-tps-c2-n5-separate",
        "compact-tps-c2-n7-separate",
        "compact-tps-c2-n10-separate",
        "compact-tps-c2-n15-separate",
        "compact-tps-c2-n20-separate",
    ])]
    fig, ax = plt.subplots(sharex=True, sharey=True)
    series = df.groupby(by="mapping", sort=False)
...
def main():
    ...
    plotSmall("data/turbine-small.csv")
    return 0
