    ...
    for group in setup["groups"]:
        for name, mapping in group["mapping"]["cases"].items():
            for constraint in group["mapping"]["constraints"]:
                for numberofpointsperrankA in as_iter(setup["general"]["numberofpointsperrank"]["A"]):
                    for numberofpointsperrankB in as_iter(setup["general"]["numberofpointsperrank"]["B"]):
                            ...
                            cases.append(
                                {
                                    ...
                                    "A": {
                                        "ranks": ranksA,
                                        "mesh": {
                                            "name": str(numberofpointsperrankA * ranksA),
                                            "file": os.path.join(
                                                test_location, f"meshA-{numberofpointsperrankA*ranksA}.vtk"
                                            ),
                                        },
                                    },
                                    "B": {
                                        "ranks": ranksB,
                                        "mesh": {
                                            "name": str(numberofpointsperrankB * ranksB),
                                            "file": os.path.join(
                                                test_location, f"meshB-{numberofpointsperrankB*ranksB}.vtk"
                                            ),
                                        },
                                    },
                                    "network": network,
...
def createRunScript(outdir, path, case):
    ...
    acmd = 'precice-aste-run -v -p A --data "{}" --mesh {} || kill 0 &'.format(case["function"], ameshLocation)
    if aranks > 1:
    ...
    bcmd = 'precice-aste-run -v -p B --data "{}" --mesh {} --output mapped || kill 0 &'.format(
    ...
    if branks == 1:
        joincmd = "[ ! -f mapped.vtu ] || mv -f mapped.vtu mapped.vtk"
        ...
        joincmd = "precice-aste-join --mesh mapped -r {} -o result.vtk".format(tmprecoveryFile)
